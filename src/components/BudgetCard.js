...
export default function BudgetCard({ name, amount, max, gray, onAddExpenseClick, onViewExpenseClick, hideButtons }) {
  const classNames = []
  if (amount > max) {
    //this overwrites the gray background
    classNames.push("bg-danger", "bg-opacity-10")
  } else if (gray) {
    //prop being passed down from the parent
    ...
    <Card className={classNames.join(" ")}>
        <Card.Body>
            ...
            {!hideButtons && <Stack direction="horizontal" gap="2" className="mt-4">
                ...
                <Button variant="outline-secondary" color="muted" onClick={onViewExpenseClick}>View Expense</Button>
            </Stack>}
        </Card.Body>
    </Card>
  )
